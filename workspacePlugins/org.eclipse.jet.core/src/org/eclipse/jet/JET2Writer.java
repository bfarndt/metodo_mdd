...
{
	// Modificado por Daniel Lucr�dio
	// m�todos de acesso para configurar o template original
	void pushTemplateFile(String templateFile);
	void popTemplateFile();
	String peekTemplateFile();
	void setGeneratedFile(String generatedFile);
	String getGeneratedFile();
	// m�todo para saltar trechos de tags
	void jump(int amount);
	void loopStart();
	void manualLoopStart();
	void loopEnd();
	void loopIterate();
	void handleDelimiter(String str, int tagStart, int tagEnd);
	void includeStart();
	void scriptletStart();
	void scriptletEnd();
	void setTag(String tag, int start, int end);
	void endTag(String tag, int amountToJump);
	void bodyContentStart();
	void bodyContentEnd();
	void addDirective(int from, int to);
	List peekDirectivesList();
